...
export class AuthService {
  ...
  login(credentials: { email: string; password: string }): Observable<{ success: boolean; token?: string; isAdmin?: boolean }> {
    ...
    console.log("🔍 [DEBUG] Sending Login Request:", credentials); // Debugging API request
  
    return this.http.post<{ token: string }>(`${this.apiUrl}/login`, credentials, { headers }).pipe(
      tap(response => console.log("✅ [DEBUG] Login API Response:", response)), // Debugging API response
      map((response) => {
        if (response.token) {
          this.saveToken(response.token);
  
          // ✅ Check if the logged-in user is an admin
          const isAdmin = credentials.email === this.getAdminEmail() && credentials.password === this.getAdminPassword();
          localStorage.setItem('isAdmin', JSON.stringify(isAdmin));
  
          console.log(`🎯 [DEBUG] Login successful! Is Admin: ${isAdmin}`);
          return { success: true, token: response.token, isAdmin };
        } else {
          console.warn("⚠️ [DEBUG] Login failed: No token received");
