...
def admin_login():
    data = request.json
    print("Received Admin Login Request:", data)  # ✅ Debugging Line

    ADMIN_EMAIL = "ibrahim.offl24@gmail.com"
    ADMIN_PASSWORD = "root123"

    if data["email"] == ADMIN_EMAIL and data["password"] == ADMIN_PASSWORD:
        print("login success")
        return jsonify({"success": True, "token": "admin-secret-token"})
    print("invalid creds")
...
def get_users_data():
    ...
    current_user = get_jwt_identity()  # ✅ Get identity from token

    if current_user != "admin":  # 🔹 Ensure identity matches "admin"
